...
def test_multipeer_deployment(tmp_path: pathlib.Path):
    ...
    conftest.processes.append(
        Popen([qdrant_exec, "--uri", bootstrap_uri], env=env, cwd=peer_dirs[0]))
    time.sleep(10)
    ...
    for i in range(1, len(peer_dirs)):
        ...
        peer_api_uris.append(get_uri(http_port))
        conftest.processes.append(
            Popen([qdrant_exec, "--bootstrap", bootstrap_uri], env=env, cwd=peer_dirs[i]))
